...
Usage:
    python3 scripts/feature_guard.py --status
    python3 scripts/feature_guard.py --gaps
    python3 scripts/feature_guard.py --stale
    python3 scripts/feature_guard.py --stale --since HEAD~5
    python3 scripts/feature_guard.py --generate security_groups
    python3 scripts/feature_guard.py --generate-all
    python3 scripts/feature_guard.py --update security_groups --test-data '{"result":"PASS",...}'
    python3 scripts/feature_guard.py --update security_groups --results-file test-results/latest-provision.json
"""
...
from agents.feature_guard import FeatureGuard, DocGapAnalyzer
from agents.feature_guard_config import FeatureGuardConfig


def cmd_status(args):
    agent = FeatureGuard(args.base_dir, verbose=args.verbose)
...
def cmd_watch(args):
    from agents.feature_guard_constants import UPSTREAM_REPO
    agent = FeatureGuard(args.base_dir, verbose=args.verbose)
...
def cmd_report(args):
    guard = FeatureGuard(args.base_dir, verbose=args.verbose)
    check_result = guard.check_all()
    html = guard.render_dashboard(check_result)
...
def cmd_run(args):
    config = FeatureGuardConfig.load(args.base_dir)

    if hasattr(args, 'auto_test') and args.auto_test:
        config.auto_test.enabled = True
    if hasattr(args, 'no_advisory_scan') and args.no_advisory_scan:
        config.advisory.enabled = False

    agent = FeatureGuard(args.base_dir, verbose=config.verbose or args.verbose)

    print("\n" + "=" * 80)
    print("  Feature Doc Lifecycle - Full Run")
    print("=" * 80)

    scan_result = None
    if config.advisory.enabled:
        print("\n[1/3] Scanning for security advisories...")
        scan_result = agent.advisory_scan()
        added = len(scan_result.get("added", []))
        errors = len(scan_result.get("errors", []))
        if added:
            print(f"      {added} new advisory(ies) added")
        elif errors:
            print(f"      Scan completed with {errors} error(s)")
        else:
            print("      No new advisories")
    else:
        print("\n[1/3] Advisory scanning disabled")

    print("\n[2/3] Checking all change sources...")
    check_result = agent.check_all(
        since=config.detection.local_since,
        record=config.auto_record,
    )

    advisory_affected = check_result.get("advisory_affected", {})
    local_count = len(check_result["local_stale"])
    upstream_count = len(check_result["upstream_stale"])
    advisory_count = len(advisory_affected)
    total_affected = len(set(
        list(check_result["local_stale"].keys()) +
        list(check_result["upstream_stale"].keys()) +
        list(advisory_affected.keys())
    ))

    print(f"      Local: {local_count} | Upstream: {upstream_count} | Advisory: {advisory_count}")
    print(f"      Total features needing attention: {total_affected}")

    test_result = None
    if config.auto_test.enabled and check_result["suggestions"]:
        feature_ids = sorted(set(
            s["feature_id"] for s in check_result["suggestions"]
        ))
        if len(feature_ids) > config.auto_test.max_features:
            print(f"\n[3/3] {len(feature_ids)} features affected, max is {config.auto_test.max_features} - skipping auto-test")
        else:
            print(f"\n[3/3] Auto-testing {len(feature_ids)} feature(s)...")
            test_result = agent.run_auto_test(
                feature_ids,
                suite_id=config.auto_test.suite_id,
            )
            status = "PASS" if test_result.get("success") else "FAIL"
            duration = test_result.get("duration", 0)
            print(f"      Result: {status} ({duration:.1f}s)")
    elif not config.auto_test.enabled:
        print("\n[3/3] Auto-testing disabled")
    else:
        print("\n[3/3] No features need testing")

    if hasattr(args, 'json') and args.json:
        result = {
            "config": {
                "auto_record": config.auto_record,
                "auto_test": config.auto_test.enabled,
                "advisory_scan": config.advisory.enabled,
            },
            "advisory_scan": scan_result,
            "check": check_result,
            "auto_test": test_result,
        }
        print(json.dumps(result, indent=2))
    else:
        if check_result["suggestions"]:
            print("\n" + "-" * 80)
            print("  Actions Taken / Needed")
            print("-" * 80)
            for s in check_result["suggestions"]:
                print(f"  {s['feature_id']} ({s['source']}): {s['action']}")
...
def main():
    ...
    run_p = sub.add_parser("run", help="Full lifecycle run: scan advisories, check changes, auto-test")
    run_p.add_argument("--auto-test", action="store_true",
                       help="Override config to enable auto-testing")
    run_p.add_argument("--no-advisory-scan", action="store_true",
                       help="Skip advisory scanning")
    run_p.add_argument("--json", action="store_true")

    report_p = sub.add_parser("report", help="Generate HTML dashboard report")
    report_p.add_argument("--html", type=str, default="feature-guard-report.html",
                          help="Output file path (default: feature-guard-report.html)")
