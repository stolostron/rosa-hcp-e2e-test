...
Uses `aws kms describe-key` to verify the KMS key exists and is enabled:
...
Asserts:
- KMS ARN format is valid (`arn:aws:kms:...`)
- KMS key exists in AWS and is in `Enabled` state
- Key ARN matches the requested value
