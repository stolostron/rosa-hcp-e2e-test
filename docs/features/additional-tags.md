...
Uses `aws resourcegroupstaggingapi get-resources` to verify tags are applied
to actual AWS resources:
...
Asserts:
- Default tags (`env`, `purpose`, `automated`) are present on AWS resources
- Custom tags match expected values if provided via `-e additional_tags={}`
