"""Tests for the FeatureDocAgent and DocGapAnalyzer."""
...
from agents.feature_doc_agent import (
    FeatureDocAgent,
    DocGapAnalyzer,
    redact,
    redact_dict,
...
class TestFeatureDocAgent:
    @pytest.fixture
    def agent(self):
        return FeatureDocAgent(BASE_DIR, verbose=False)

    def test_init(self, agent):
        assert agent.name == "FeatureDoc"
    ...
    def test_render_live_test_table(self, agent):
        data = {"date": "2026-06-10", "provision_result": "PASS", "region": "us-west-2"}
        table = agent._render_live_test_table(data)
        assert "## Live Test Record" in table
        assert "2026-06-10" in table
        assert "PASS" in table
        assert "us-west-2" in table

    def test_render_live_test_table_redacted(self, agent):
        data = redact_dict({
            "date": "2026-06-10",
            "vpc": "vpc-abc12345",
            "sg": "sg-def67890",
        ...
        table = agent._render_live_test_table(data)
        assert "vpc-abc12345" not in table
        assert "sg-def67890" not in table
...
class TestUpstreamImpact:
    ...
    def test_parse_pr_url(self):
        repo, num = FeatureDocAgent._parse_pr_url(
    ...
    def test_parse_pr_url_invalid(self):
        repo, num = FeatureDocAgent._parse_pr_url("https://example.com/foo")
        assert repo is None
        assert num is None
...
class TestCheckUpstream:
    ...
    def agent(self, tmp_path):
        a = FeatureDocAgent(BASE_DIR, verbose=False)
    ...
    def test_check_upstream_up_to_date(self, agent):
        ...
        with patch.object(FeatureDocAgent, '_fetch_branch_sha', return_value="abc123"):
    ...
    def test_check_upstream_new_changes(self, agent):
        ...
        with patch.object(FeatureDocAgent, '_fetch_branch_sha', return_value="new_sha"), \
             patch.object(FeatureDocAgent, '_fetch_commit_diff',
    ...
    def test_check_upstream_first_run(self, agent):
        with patch.object(FeatureDocAgent, '_fetch_branch_sha', return_value="sha123"), \
             patch.object(FeatureDocAgent, '_fetch_recent_pr_files',
    ...
    def test_check_upstream_saves_sha(self, agent):
        with patch.object(FeatureDocAgent, '_fetch_branch_sha', return_value="sha999"), \
             patch.object(FeatureDocAgent, '_fetch_recent_pr_files', return_value=[]):
    ...
    def test_check_upstream_marks_features_stale(self, agent):
        ...
        with patch.object(FeatureDocAgent, '_fetch_branch_sha', return_value="new"), \
             patch.object(FeatureDocAgent, '_fetch_commit_diff',
    ...
    def test_check_upstream_error_no_sha(self, agent):
        with patch.object(FeatureDocAgent, '_fetch_branch_sha', return_value=None):
            result = agent.check_upstream(repo="test/repo", branch="main")
...
class TestAdvisories:
    ...
    def test_match_advisory_keywords_etcd(self):
        matched = FeatureDocAgent._match_advisory_to_features(
...
class TestAdvisoryScanning:
    ...
    def test_map_redhat_severity(self):
        assert FeatureDocAgent._map_redhat_severity("critical") == "critical"
        assert FeatureDocAgent._map_redhat_severity("important") == "high"
        assert FeatureDocAgent._map_redhat_severity("moderate") == "medium" 
        assert FeatureDocAgent._map_redhat_severity("low") == "low"
        assert FeatureDocAgent._map_redhat_severity("unknown") == "medium"
        assert FeatureDocAgent._map_redhat_severity("") == "medium"
